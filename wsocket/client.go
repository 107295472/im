...
func transitData(message []byte, userID int64, client *Client) error {
    ...
    //    return err
    //}
    parseData, err :=api.ApiParse(message,[]byte("sdfsdf"))
    if err != nil {
        util.LogError().Println("api parse error:", err)
    }
    sendUsers:=parseData.SendUsers()
    return X2cSend(client.serverName, sendUsers, message, client)
}
...
func serveWs(hub *Hub, w http.ResponseWriter, r *http.Request) {
    ...
    header := http.Header {"User-Agent" : {"im"}}
